...
export class MainScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.statusText = this.add.text(16, 190, '', {
      fontFamily: 'Arial',
      fontSize: '16px',
      color: '#111827',
      backgroundColor: '#ffffffcc',
    ...
    this.menuBg = this.add.rectangle(0, 0, 320, 180, 0x111827, 0.92)
      .setOrigin(0, 0)
      .setVisible(false)
      .setDepth(200);

    this.menuText = this.add.text(0, 0, '', {
      fontFamily: 'Arial',
      fontSize: '18px',
      color: '#f8fafc',
      lineSpacing: 8
  ...
  resolveAutoNeeds() {
    if (this.catState.shouldAutoVomit(this.time.now)) {
      ...
      this.score += points;
      this.lastStatusMessage = `Кота вырвало на ${targetLabel}. (+${points})`;
    }

    if (this.catState.shouldAutoPee(this.time.now)) {
      const points = this.catState.getFloorPeePoints();

      this.catState.triggerAutoPee();
      this.score += points;
      this.lastStatusMessage = `Кот не выдержал и написал на пол. (+${points})`;
    }
  }
  ...
  getCurrentZoneForAuto() {
    ...
    for (const zone of this.zoneObjects) {
      ...
      const dist = Phaser.Math.Distance.Between(this.cat.x, this.cat.y, zone.x, zone.y);
      if (dist < nearestDist) {
        nearest = zone;
        nearestDist = dist;
      }
  ...
  handleActionInput() {
    ...
    if (Phaser.Input.Keyboard.JustDown(this.keys.action1)) this.startActionByIndex(0);
    if (Phaser.Input.Keyboard.JustDown(this.keys.action2)) this.startActionByIndex(1);
    if (Phaser.Input.Keyboard.JustDown(this.keys.action3)) this.startActionByIndex(2);
    if (Phaser.Input.Keyboard.JustDown(this.keys.action4)) this.startActionByIndex(3);
  }
  ...
  updateHud() {
    ...
    } else if (this.catState.
      peePending && this.catState.canPee) {
  ...
  updateActionMenu() {
    ...
    const lines = this.currentActions.length
      ? this.currentActions.map((action, i) => `[${i + 1}] ${action.label} (${(action.durationMs / 1000).toFixed(1)}с)`)
  ...
  redrawZones() {
    ...
    for (const zone of this.zoneObjects) {
      ...
      if (zone.type === 'ellipse') {
        ...
        this.zoneGraphics.strokeEllipse(zone.x, zone.y, zone.width, zone.height);
      }
    }
  }
  ...
  createZoneLabels() {
    this.zoneLabels = this.zoneObjects.map((zone) => {
      return this.add.text(zone.x, zone.y, zone.label, {
        fontFamily: 'Arial',
        fontSize: '16px',
        color: '#166534',
